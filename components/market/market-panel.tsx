...
function PriceChart({
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Weekly egg price trends"
    >
...
export function MarketPanel({ data, compact = false }: Props) {
  ...
  const chartSeries = useMemo(
    () =>
      data.time_series.series.filter(
        (s) =>
          s.price_level === priceLevel && systems.includes(s.name),
      ),
  ...
  const insights = useMemo(
    () =>
      data.insights.filter(
        (i) =>
          i.price_level === priceLevel && systems.includes(i.system),
      ),
  ...
  return (
    <div
      className={`w-full bg-white text-foreground ${compact ? "p-4" : "p-5 md:p-8"} rounded-2xl`}
    >
      <header className="mb-6 md:mb-8">
        ...
        <h1 className="font-serif text-2xl md:text-3xl font-bold tracking-tight">
          Vietnam egg price signals
        </h1>
        <p className="mt-2 text-sm md:text-base text-muted-foreground max-w-2xl">
          Observed Farmgate and Retail prices for Caged and Cage-Free eggs.
          Figures are descriptive signals, not forecasts or guaranteed returns.
        </p>
        <p className="mt-2 text-xs text-muted-foreground">
          Coverage {data.meta.coverage.start} → {data.meta.coverage.end}
          {data.meta.source === "mock" ? " · Sample data" : null}
        </p>
      </header>
        ...
        {summary.map((row) => (
          ...
          >
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {row.observations.toLocaleString()} observations · {row.days} days
            </p>
          </article>
        ))}
      </section>
      ...
      <section className="rounded-2xl border p-4 md:p-5 mb-8">
        <div className="flex flex-wrap items-end justify-between gap-3 mb-4">
          <div>
            <h2 className="font-serif text-xl font-bold">Price trend</h2>
            <p className="text-sm text-muted-foreground">
              {data.time_series.interval} average · {priceLevel}
            </p>
          </div>
        ...
        {chartSeries.length ? (
          ...
          <p className="text-sm text-muted-foreground py-10 text-center">
            Select at least one production system to view trends.
          </p>
        )}
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-2 mb-8">
        <article className="rounded-2xl border p-5 bg-[#F9F8F3]">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            Matched {data.matched_comparison.housing.baseline} vs{" "}
            {data.matched_comparison.housing.target}:{" "}
        ...
        <article className="rounded-2xl border p-5 bg-[#1B2A4A] text-white">
          ...
          <p className="text-sm text-white/75 mt-2">
            Matched spread across{" "}
            {data.matched_comparison.supply_chain.cells} cells ·{" "}
            {data.matched_comparison.supply_chain.days} days
          </p>
        </article>
      </section>
      ...
      {insights.length > 0 && (
        <section>
          <h2 className="font-serif text-xl font-bold mb-1">
            What this may mean for your farm
          </h2>
      ...
      {!compact && (
        <p className="mt-8 text-xs text-muted-foreground text-center">
          HealthyFarm · Laying Hen Welfare Network · Prices in VND per egg
        </p>
      )}
    </div>
  );
